...
function officerBonus(){ let b=1; if(state.officers.geologist) b*=1.10; return b; }
...
function energyStats(p){const solar=defs.buildings.solarPlant.power(p.buildings.solarPlant); const satellites=(p.ships.solarSatellite||0)*20; const prod=solar+satellites; const use=defs.buildings.metalMine.powerUse(p.buildings.metalMine)+defs.buildings.crystalMine.powerUse(p.buildings.crystalMine)+defs.buildings.deutSynth.powerUse(p.buildings.deutSynth); return {prod,use,ratio: use? Math.min(1,prod/use):1};}
function hourly(p){const e=energyStats(p).ratio; return {metal: defs.buildings.metalMine.prod(p.buildings.metalMine)*e, crystal: defs.buildings.crystalMine.prod(p.buildings.crystalMine)*e, deut: defs.buildings.deutSynth.prod(p.buildings.deutSynth)*e}}
function maxStorage(p){return {metal:5000*p.buildings.metalStorage, crystal:5000*p.buildings.crystalStorage, deut:5000*p.buildings.deutTank}}
...
function tick(){
  ...
  renderTop(); renderSide(); renderView(false);
}
...
function viewBuildings(){ const p=active(); return `<h2>Gebäude</h2><div class="list">${Object.entries(defs.buildings).filter(([,d])=>!d.isDefense && !d.facility).map(([k,d])=>{ const lvl=(p.buildings[k]||0)+1; const c=scaledCost(d.base,lvl); const ok=meetsRequirements(p,d.requires); return `<div class="row"><div><strong>${d.name}</strong><div class="sub">Stufe ${p.buildings[k]||0}</div><div class="sub">Kosten: M ${fmt(c.metal)} · K ${fmt(c.crystal)} · D ${fmt(c.deut)}</div>${!ok?`<div class="sub warn-text">Benötigt: ${requirementText(d.requires)}</div>`:''}</div><button class="btn" data-build="${k}" ${ok?'':'disabled'}>Ausbauen</button></div>`; }).join('')}</div>`; }

function viewFacilities(){ const p=active(); const facKeys = Object.entries(defs.buildings).filter(([,d])=>d.facility); return `<h2>Anlagen</h2><div class="list">${facKeys.map(([k,d])=>{ const lvl=(p.buildings[k]||0)+1; const c=scaledCost(d.base,lvl); const ok=meetsRequirements(p,d.requires); return `<div class="row"><div><strong>${d.name}</strong><div class="sub">Stufe ${p.buildings[k]||0}${d.moonOnly?' · nur auf Monden':''}</div><div class="sub">Kosten: M ${fmt(c.metal)} · K ${fmt(c.crystal)} · D ${fmt(c.deut)}</div>${!ok?`<div class="sub warn-text">Benötigt: ${requirementText(d.requires)}</div>`:''}</div><button class="btn alt" data-build="${k}" ${ok?'':'disabled'}>Ausbauen</button></div>`; }).join('')}</div>`; }
...
function viewDefense(){ const p=active(); const defenseKeys = Object.entries(defs.buildings).filter(([,d])=>d.isDefense); return `<h2>Verteidigung</h2><div class="list">${defenseKeys.map(([k,d])=>{ const count=p.buildings[k]||0; const ok=meetsRequirements(p,d.requires); return `<div class="row"><div><strong>${d.name}</strong><div class="sub">Vorhanden ${fmt(count)} · Angriff ${d.attack} · Hülle ${fmt(d.hull)}</div><div class="sub">Kosten: M ${fmt(d.base.metal)} · K ${fmt(d.base.crystal)} · D ${fmt(d.base.deut)}</div>${!ok?`<div class="sub warn-text">Benötigt: ${requirementText(d.requires)}</div>`:''}</div><button class="btn danger" data-defense="${k}" ${ok?'':'disabled'}>Bauen</button></div>`; }).join('')}</div>`; }
...
function viewSettings(){ return `<h2>Einstellungen</h2><div class="grid2"><div class="card"><h3>Spielstand exportieren</h3><div class="small">Speichert den aktuellen Zustand als JSON-Datei zum Download.</div><div style="height:10px"></div><button class="btn" id="saveBtn">Spielstand speichern</button></div><div class="card"><h3>Spielstand laden</h3><div class="small">Lädt eine zuvor exportierte JSON-Datei.</div><div style="height:10px"></div><input type="file" id="loadInput" accept="application/json"></div></div>`; }
...
function viewMarket(){ return `<h2>Markt</h2><div class="market-grid"><div class="card"><div class="label">Metall</div><div class="value">1.0</div></div><div class="card"><div class="label">Kristall</div><div class="value">1.5</div></div><div class="card"><div class="label">Deuterium</div><div class="value">3.0</div></div></div><div style="height:16px"></div><div class="grid2"><div class="card"><h3>Ressourcen handeln</h3><form class="market-form" id="marketForm"><label>Abgeben<select name="give"><option value="metal">Metall</option><option value="crystal">Kristall</option><option value="deut">Deuterium</option></select></label><label>Erhalten<select name="want"><option value="crystal">Kristall</option><option value="metal">Metall</option><option value="deut">Deuterium</option></select></label><label>Menge<input type="number" min="1" value="100" name="amount"></label><button class="btn good" type="submit">Am Markt tauschen</button></form></div><div class="card"><h3>Raten</h3><div class="small">1 Metall = 1 Wert, 1 Kristall = 1.5, 1 Deuterium = 3. Marktgebühr 10%.</div></div></div>`; }
...
ensureAllDefaults(); render(); setInterval(tick,1000); setInterval(render,5000);
